locals {
  ...
  game_run_commands = join("\n\n", [
    for game_name, game in var.games : join(" \\\n  ", concat(
      ["# --- ${game_name} ---\n${join("\n", [for v in game.volumes : "docker volume create ${game_name}-${v.name_suffix}"])}\n${game_name == "vrising" ? "mkdir -p /opt/game-configs/vrising\nif [ ! -f /opt/game-configs/vrising/ServerHostSettings.json ]; then\n  echo \"WARNING: /opt/game-configs/vrising/ServerHostSettings.json not found; container may fail until valid JSON is uploaded\"\n  touch /opt/game-configs/vrising/ServerHostSettings.json\nfi\nif [ ! -f /opt/game-configs/vrising/ServerGameSettings.json ]; then\n  echo \"WARNING: /opt/game-configs/vrising/ServerGameSettings.json not found; container may fail until valid JSON is uploaded\"\n  touch /opt/game-configs/vrising/ServerGameSettings.json\nfi\n" : ""}docker run"],
      ["--detach"],
      ["--restart unless-stopped"],
      ["--name ${game_name}-server"],
      [for v in game.volumes : "--mount type=volume,source=${game_name}-${v.name_suffix},target=${v.container_path}"],
      game_name == "vrising" ? [
        "--mount type=bind,source=/opt/game-configs/vrising,target=/mnt/vrising/persistentdata/Settings"
      ] : [],
